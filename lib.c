...
{
  unsigned int i;
  for (i = 0; i < t->size && (t->symbols[i].u.value != v || t->symbols[i].kind != CONSTANT); i++)
    ;
  if (i == t->size)
  {
...
{
  unsigned int i;
  for (i = 0; i < t->size && (t->symbols[i].u.value != v || t->symbols[i].kind != FCONST); i++)
    ;
  if (i == t->size)
  {
    if (t->size == t->capacity)
...
void symtable_dump(struct symtable *t)
{
  ...
  for (i = 0; i < t->size; i++)
  {
    ...
    if (t->symbols[i].kind == NAME_INT || t->symbols[i].kind == NAME_FLOAT ||
        t->symbols[i].kind == NAME_STR || t->symbols[i].kind == ADDR_INT || t->symbols[i].kind == ADDR_FLOAT)
...
static void quad_dump(struct quad *q, FILE *out)
{
  ...
  case PLUS:
    ...
    if ((q->sym1->kind == CONSTANT || q->sym1->kind == NAME_INT || q->sym1->kind == ADDR_INT) &&
        (q->sym2->kind == CONSTANT || q->sym2->kind == NAME_INT || q->sym2->kind == ADDR_INT) &&
        (q->sym3->kind == CONSTANT || q->sym3->kind == NAME_INT || q->sym3->kind == ADDR_INT))
      fprintf(out, "\nadd $t0, $t1, $t2\n");
    else if ((q->sym1->kind == FCONST || q->sym1->kind == NAME_FLOAT || q->sym1->kind == ADDR_FLOAT) &&
             (q->sym2->kind == FCONST || q->sym2->kind == NAME_FLOAT || q->sym2->kind == ADDR_FLOAT) &&
             (q->sym3->kind == FCONST || q->sym3->kind == NAME_FLOAT || q->sym3->kind == ADDR_FLOAT))
      fprintf(out, "\nadd.s $f0, $f1, $f2\n");
    else
    {
  ...
  case U_MINUS:
    ...
    if ((q->sym1->kind == CONSTANT || q->sym1->kind == NAME_INT || q->sym1->kind == ADDR_INT) &&
        (q->sym2->kind == CONSTANT || q->sym2->kind == NAME_INT || q->sym1->kind == ADDR_INT))
      fprintf(out, "\nsub $t0, $t2, $t1\n");
    else if ((q->sym1->kind == FCONST || q->sym1->kind == NAME_FLOAT || q->sym1->kind == ADDR_FLOAT) &&
             (q->sym2->kind == FCONST || q->sym2->kind == NAME_FLOAT || q->sym2->kind == ADDR_FLOAT))
      fprintf(out, "\nsub.s $f0, $f2, $f1\n");
  ...
  case INIT_WHILE:
  { // obligé de mettre {} parce que le c n'accepte pas de commencer par une déclaration
    int true = peek(&stack);
    loadsym(q->sym1, out, 0);
    fprintf(out, "\nbeqz $t0, L%d\n", true);
    break;
